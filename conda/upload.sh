...
if [ -n "${TRAVIS_TAG}" ]; then
  ...
  if [ ${TRAVIS_EVENT_TYPE} = "cron" ]; then
    conda install anaconda-client
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} -l nightly ~/miniconda/conda-bld/linux-64/apsw-*.tar.bz2 --force
  else
    conda install anaconda-client
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} -l edge ~/miniconda/conda-bld/linux-64/apsw-*.tar.bz2 --force
  fi
else
  exit 0
fi
